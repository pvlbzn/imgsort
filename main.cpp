...
void test();
uint8_t* sort(std::function<uint8_t* (uint8_t*, int, int, bool)> fn, uint8_t* in, int len, int k, bool reverse);
...
void test() {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        image_pool.push_back(Image(ss.str(), true));
    }

    int counter = 1;
    for (auto& image : image_pool) {
        auto vec = image.vectorize_mat();
        auto svec = sort(Image::sort, vec, image.get_rows() * image.get_cols(), 256, true);
